...
import { AppModule } from './modules/decorators/memcached-store/app.module';
import { CACHEABLE, Cache } from '../../src';
import { TestService } from './modules/decorators/memcached-store/test/test.service';

describe('Decorator For Memcached Store', () => {
  let server: Server;
  ...
  describe('CacheEvict Decorator', () => {
    ...
    it('should throw error delete all results', async () => {
      const numbers = [];
      for (let i = 1; i <= 10; i++) {
        numbers.push(i);
      }
      ...
      await request(server).delete('/value').expect(500);

      for (const num of numbers) {
        await expect(
          cacheable.get(`value:${num}`),
